...
## Reduced sampling frequency (LOOCV), 12 days ahead
...
```{r GRE_RF_subsample1day12steps, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE}
set.seed(1)
...
```{r GRE_RF_subsample3days4steps, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE}
set.seed(1)
...
#### Random Forest
```{r GRE_RF_subsample6days2steps, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE}
set.seed(1)
...
```{r GRE_RF_subsample12days1step, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE}
set.seed(1)
...
### Controlled time window (i.e. varying sample size)
...
#### Daily sampling, 12 steps ahead, Multiview EDM

```{r GREForecast_Freq1over1_12steps_unreduced, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE}
set.seed(1)
...
#### 3 days sampling interval, 4 steps ahead, Multiview EDM

```{r GREForecast_Freq1over3_4steps_unreduced, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE}
set.seed(1)
...
#### 6 days sampling interval, 2 steps ahead, Multiview EDM

```{r GREForecast_Freq1over6_2steps_unreduced, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE}
set.seed(1)
...
#### 12 days sampling interval, 1 step ahead, Multiview EDM

```{r GREForecast_Freq1over12_1step_unreduced, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE}
set.seed(1)
...
### Forecast horizon analysis: 24-days ahead forecasts

#### Daily sampling, 24 steps ahead, Multiview EDM

```{r ForecastHorizon_dailyData, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE}
set.seed(1)
...
#### 3 Days sampling interval, 8 steps ahead, Multiview EDM

```{r ForecastHorizon_3daysFreq, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE}
set.seed(1)
...
#### 6 Days sampling interval, 4 steps ahead, Multiview EDM

```{r ForecastHorizon_6daysFreq, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE}
set.seed(1)
...
#### 12 Days sampling interval, 2 steps ahead, Multiview EDM

```{r ForecastHorizon_12daysFreq, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE}
set.seed(1)
...
## Reduced time points forecasting, 12 days ahead
...
```{r Figure2, fig.height=11.5, fig.width=10, warning=FALSE, dev=c('png', 'pdf', 'svg', 'tiff')}
...
(p.a+p.b.d+plot_layout(widths = c(2,1)))/
  ...
  p.leg + plot_layout(heights = c(4,4,4,1)) + plot_annotation(title = "\n\nFigure 2", )
```

## Interaction figure 

```{r Figure3, fig.height=10.5, fig.width=10, warning=FALSE, dev=c('png', 'pdf', 'svg', 'tiff')}
# numint, freq
...
(p2.a + p2.b + p2.e + p2.f + p2.c + p2.d + plot_layout(ncol = 2))/p.leg + plot_layout(heights = c(12,1)) + plot_annotation(title = "\n\nFigure 3")
```
...
```{r Figure4, fig.height=7.5, fig.width=8, warning=FALSE, dev=c('png', 'pdf', 'svg', 'tiff')}
...
fig4a2.tp.2 <- dd.slopes.tp %>%
  ...
  labs(y="Slope between the number of\ninteractions and forecast error", x="Proportion of time\npoints", tag = "c")+
  ylim(-0.075,0.08) +
  theme(axis.title.x = element_blank())
...
fig4a2.2 + fig4b2.2 + fig4a2.tp.2 + fig4b2.tp.2 + fig4Daug1 + fig4Daug2 + plot_layout(ncol = 2, heights = c(4,4,1)) + plot_annotation(title = "\n\nFigure 4")
```
...
```{r ciliates, fig.height=3, fig.width=8, include=FALSE}
presiplot <- ddHF_GRE2_plot2 %>%
  dplyr::filter(Name=="Ciliates") 

presiplot <- presiplot%>%
  group_by(subgroup, group, Name) %>%
  mutate(d.since.start = date - min(presiplot$date))



presiplot %>%
  ggplot(aes(date, value))+
  geom_line(col="#e41a1c", size=0.5) +
  # geom_line(data = presiplot %>%dplyr::filter(d.since.start %in% seq(0,2000,by=30)), col="black")+
  geom_line(data = presiplot %>%dplyr::filter(d.since.start %in% seq(0,2000,by=14)), col="#377eb8", size=0.7)+
  standard_theme() +
  theme(axis.ticks.y = element_blank(),
        axis.text.y = element_blank(),
        axis.text.x = element_text(size=14),
        axis.title = element_text(size=18))+
  labs(y="Ciliates abundance", x="Year")
...
### S2.4:  Robustness analyses

#### S2.4.1: Random Forest forecast

##### Figure: Random Forest forecast
...
#### S2.4.2: Controlled time window

##### Figure: Controlled time window
...
#### S2.4.3: Forecast horizon, 24 days ahead forecasts

##### Figure: Forecast horizon, 24 days ahead forecasts
...
##### Table: Forecast horizon, 24 days ahead forecasts
